...
// UtilBattleDataから継承 
// 計算()
UtilBattleData.calc = function (gameData, userData, options, userType) {
    // 変数の初期化
    const enemy = options.enemyData; // 敵データ
    const user = userData; // 自データ
    ...
    if (userType === 'user') {
        ...
        // 必要MPに対してMPが足りているか
        if (user.mp < mp) {
            heal = damage = 0; // 足りていないので効果なし
        } else {
            if (heal) {
                // 回復
                user.hp = minMax(1, user.hp + heal, user.hpMax); // 自回復
            } else {
                // ダメージ
        ...
        damage = (at + random() % at) - (random() % df);
        damage = minMax(1, damage | 0, 999); // 範囲内の整数に
        user.hp = minMax(0, user.hp - damage, 999); // 自ダメージ
    }
